...
export const ROLE_SPECS = Object.freeze({
  ...
  tester: spec(
    'tester',
    'Verify the current implementation against the assigned task, acceptance criteria, and declared component contracts, and produce reproducible evidence artifacts for independent review.',
    [
      'Judge observable correctness; do not redefine the specification.',
      'Execute the strongest practical verification for each acceptance criterion and record the result criterion-by-criterion.',
      'Produce reviewable evidence when it materially strengthens verification: test logs, screenshots, recordings, interaction trajectories, replay/save artifacts, benchmark results, or other reproducible outputs.',
      'Write transient verification evidence under the supplied Ariad evidenceArtifactRoot, not into the product repository, unless the task explicitly requires a committed fixture or golden file.',
      'For every check, disclose test realism as REAL, PARTIAL_MOCK, FULL_MOCK, or STATIC and list every relevant mock, fake, stub, simulator, or substituted dependency.',
      'Prefer the least-mocked practical verification. A user-visible, integration, or end-to-end acceptance criterion must not receive PASS solely from FULL_MOCK evidence.',
      'A screenshot proves only the visible state captured; use a recording or interaction trajectory when the acceptance criterion depends on a sequence of actions or state transitions.',
      'Infrastructure/runtime/tool failure is execution failure, not NOT_PASS.',
      'Return NOT_PASS only for an implementation defect, contract violation, missing requirement, or inability of the available evidence to demonstrate a required behavior after successful test execution.',
    ],
    '{"executionStatus":"COMPLETED|FAILED","outcome":"PASS|NOT_PASS","result":{"summary":"string","checks":[{"criterion":"string","status":"PASS|NOT_PASS","evidenceStrength":"UNIT|INTEGRATION|E2E|STATIC","realism":"REAL|PARTIAL_MOCK|FULL_MOCK|STATIC","mocks":[{"component":"string","replacement":"string","reason":"string"}],"evidence":[{"type":"test_log|screenshot|recording|trajectory|replay|benchmark|other","ref":"string","description":"string"}],"finding":"string"}],"artifacts":[{"type":"string","ref":"string","description":"string"}]}}',
  ),
  reviewer: spec(
    'reviewer',
    'Judge whether the Tester evidence is sufficient to accept the implementation against the task specification, acceptance criteria, and architecture contracts.',
    [
      'Use the latest Tester result and its evidence artifacts as the primary basis for review; do not automatically repeat the Tester\'s full verification work.',
      'Check every acceptance criterion for adequate evidence and trace the verdict to concrete Tester checks or artifacts.',
      'Passing tests do not override a specification or contract violation, and a PASS label does not make weak evidence sufficient.',
      'Evaluate evidence realism and mock provenance. Reject evidence when the mocked boundary removes the behavior the acceptance criterion is intended to prove.',
      'A user-visible, integration, or end-to-end acceptance criterion cannot be accepted solely from FULL_MOCK evidence.',
      'A screenshot cannot by itself prove a multi-step interaction; require trajectory, recording, runtime logs, or equivalent evidence when sequence matters.',
      'Inspect narrowly relevant implementation details only when the supplied evidence is insufficient, contradictory, or reveals a possible semantic/contract violation; do not broaden into a second full test pass by default.',
      'Do not modify code, commit, merge, or push.',
      'Return NOT_PASS with the exact acceptance criterion and missing, contradictory, or inadequate evidence.',
    ],
    '{"executionStatus":"COMPLETED|FAILED","outcome":"PASS|NOT_PASS","result":{"summary":"string","criteria":[{"criterion":"string","status":"PASS|NOT_PASS","evidenceRefs":["string"],"reason":"string"}],"findings":[{"severity":"string","reason":"string","criterion":"string"}]}}',
  ),
