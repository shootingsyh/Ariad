...
test('fourth developer entry is blocked by project debugger', async()=>{const exec=new ScriptedFakeExecutor({developer:[pass('IMPLEMENTATION_READY'),pass('IMPLEMENTATION_READY'),pass('IMPLEMENTATION_READY')],tester:[pass('NOT_PASS'),pass('NOT_PASS'),pass('NOT_PASS')],project_debugger:[pass('TASK_TOO_LARGE')],pm:[pass('REPLAN_READY')]});const result=await new WorkflowEngine(exec).runFeature('F4');assert.equal(result.status,'WAITING_REPLAN');assert.equal(exec.calls.filter(c=>c.role==='developer').length,3);assert.equal(exec.calls.at(-2)?.role,'project_debugger');});
...
test('TASK_CONTRADICTORY routes to PM and preserves history', async()=>{const exec=new ScriptedFakeExecutor({developer:[pass('IMPLEMENTATION_READY'),pass('IMPLEMENTATION_READY'),pass('IMPLEMENTATION_READY')],tester:[pass('NOT_PASS'),pass('NOT_PASS'),pass('NOT_PASS')],project_debugger:[pass('TASK_CONTRADICTORY')],pm:[pass('NEEDS_HUMAN')]});const result=await new WorkflowEngine(exec).runFeature('F6');assert.equal(result.status,'NEEDS_HUMAN');assert.equal(result.history.some(e=>e.role==='project_debugger'),true);});
...
test('source-control finalizer runs exactly once and only after reviewer PASS', async()=>{
  ...
  const exec=new ScriptedFakeExecutor({
    developer:[pass('IMPLEMENTATION_READY'),pass('IMPLEMENTATION_READY')],
    tester:[pass('PASS'),pass('PASS')],
    reviewer:[pass('NOT_PASS'),pass('PASS')]
  });
