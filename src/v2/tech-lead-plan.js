...
export function validateTechLeadPlan(plan) {
  ...
  for (let index = 0; index < plan.tasks.length; index += 1) {
    ...
    if (task.verification != null) {
      if (!Array.isArray(task.verification)) fail(`${path}.verification`, 'must be an array');
      const seenVerification = new Set();
      for (let vIndex = 0; vIndex < task.verification.length; vIndex += 1) {
        const entry = task.verification[vIndex];
        const vPath = `${path}.verification[${vIndex}]`;
        ...
        if (criterionIndex < 0 || criterionIndex >= task.acceptanceCriteria.length) fail(`${vPath}.criterionId`, 'references missing acceptance criterion');
