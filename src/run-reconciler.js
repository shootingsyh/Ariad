export class RunReconciler {
  constructor({ store, registry, emit = () => {} }) {
    if (!store || typeof store.listRecoverable !== 'function') {
      throw new Error('RunReconciler requires a run store with listRecoverable()');
    }
    if (!registry || typeof registry.get !== 'function') {
      throw new Error('RunReconciler requires a runtime registry');
    }
    this.store = store;
  ...
  async reconcile() {
    ...
    for (const run of this.store.listRecoverable()) {
      if (run.state === 'CREATED' && !run.externalId) {
        const event = { type: 'RUN_READY_TO_REDISPATCH', runId: run.id, runtimeKey: run.runtimeKey };
      ...
      if (result?.state === 'LOST') {
        this.store.update(run.id, {
          state: 'LOST',
          failure: { kind: 'RUN_LOST', detail: result },
        });
      ...
      if (result?.state) {
        this.store.update(run.id, { state: result.state });
      }
      const event = {
        type: 'RUN_RECONCILED_ACTIVE',
        runId: run.id,
        runtimeKey: run.runtimeKey,
